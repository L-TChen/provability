...
\ltnote{(mere) proposition, set, structure vs.\ property, $n$-types are closed under $\sum$, $\prod$-type formers, \setU, \propU, function extensionality}
...
Similarly, the notion of natural transformations is introduced for exposures which shares the same idea of the ordinary natural transformation but subject to~$\sim$ only:
\begin{definition}
  Given exposures $P, Q\colon \mathbf{C} \looparrowright \mathbf{D}$, a \emph{natural transformation of exposures} $t\colon P \to Q$ is a family of well-defined morphisms $t_X \colon PX \to QX$ such that the naturality holds up to~$\sim$ in the sense that $Qf \circ t_X \sim t_Y \circ Pf$ for every well-defined morphism~$f\colon X \to Y$. 
\end{definition}
...
In our framework, however, we would have to adopt \emph{oplax} natural transformations for the comultiplication, where the naturality holds only up to $\reduce$.
...
However, with functional extensionality, any inhabitant of~\eqref{eq:alternative-right-totality} leads to a contradiction~\cite{Troelstra1977}. \ltnote{prove it formally}
...
\begin{example}
  ...
  \]
  is an assembly where $r_\Nat$ and $t_\Nat$ are similar to those of~\Cref{ex:assembly-lambda}.  
\end{example}
...
\hsnote{Which is based on (a) PCA?}
...
\begin{example} \label{ex:global-element-of-unit}
  One has a global element $\bigstar\colon \top \to \boxtimes \top$ corresponding to the designated inhabitant~$\star : \Unit$ given by $(\mathtt{I}, \star, \refl_\twoheadrightarrow)$ with a realiser $\qquote{M}$ using \Cref{lem:global-element}. 
...
\end{example}
The above example also shows that $\boxtimes \top$ cannot be $\PP$-isomorphic to $\top$, since there are countably many representations for $\star : \Unit$, i.e.\ inhabitants of $\bproj{\boxtimes \top}$ while there is exactly only one inhabitant of $\bproj{\top} \equiv \Unit$. 
Similarly, $\boxtimes (X \times Y)$ is not $\PP$-isomorphic to $\boxtimes X \times \boxtimes Y$.
...
In addition, there is also a family of maps $\delta_X : \boxtimes X \to \boxtimes \boxtimes X$ tracked by~$\mathtt{quote}$ corresponding to the quotation of a quoted term where $\bproj{\delta_X}(M, x, r) \defeq (\qquote{M}, (M, x, r), \refl_\twoheadrightarrow)$. 
For any global element $a\colon \top \to \boxtimes X$ given by~\Cref{lem:global-element} whose tracker is indeed a closed term, we can see that $\delta_X \circ a$ coincides with $\boxtimes a \circ \bigstar$:
\[
  \delta_X(a(\star)) = \delta_X(M, x, r) = (\qquote{M}, (M, x, r), \refl_\twoheadrightarrow)
  = \boxtimes a(\mathtt{I}, \star, \refl_\twoheadrightarrow) = \boxtimes a(\bigstar(\star)).
\]
\ltnote{is every global element of $\boxtimes X$ tracked by a closed term?}
...
However, unlike Theorem~11 of Kavvos~\cite{Kavvos2017a}, these maps $\delta_X$ are unfortunately \emph{not} natural in~$X$.
...
where $\mathsf{subst}_{\twoheadrightarrow}$ is the witness of the reduction $\mathsf{subst}\,\qquote{F}\,\qquote{M} \reduce \qquote{F[M]}$.
Yet we may consider the reduction relation $\mathsf{subst}\,\qquote{F}\,\qquote{M} \reduce \qquote{F[M]}$ as a partial order.
In detail, for inhabitants of $\boxtimes X$ define $(M, x, r) \leq (N, y, s)$ if $M \reduce N$ and $x = y$, and
accordingly define $f \leq g$ if $\bproj{f}\,x \leq \bproj{g}\,x$.
Then we have $\bproj{\boxtimes \boxtimes f \circ \delta_X} \leq \bproj{\delta_Y \circ \boxtimes f}$.
That is, $\delta$ is a \emph{lax natural transformation} up to $\sim$.
\end{remark}

The above remark suggests a theory on categories enriched not by $\PP$-sets but \emph{partially ordered $\PP$-sets}, which is beyond the scope of this paper, so we refrain ourselves from further discussion.
...
\begin{remark} \label{remark:difference-boxtimes}
  ...
  Preserving finite products are considered \emph{unsatisfactory} because intensions, i.e.\ elements of a PCA, are subject to certain equations already as those representations are identified on the level of PCA.
  By taking $\lambda$-terms only subject to $\alpha$-equivalence as realisers, we are able to distinguish computationally equivalent intensions.
...
\begin{theorem}\label{thm:S4-no-quoting}
  There is no family of trackable maps from $X$ to $\boxtimes X$ indexed by assemblies~$X$ satisfying
  \begin{equation} \label{eq:naturality-quoting}
    \bproj{\boxtimes f \circ \eta_X}\,x \leq \bproj{\eta_Y \circ f}\,x
  \end{equation}
  for each trackable map $f\colon X \to Y$ where $(M, x, r) \leq (N, y, s)$ if $x = y$ and $M \reduce N$.
\end{theorem}
...
\begin{proof}
  Assume that such a family of maps $\eta_X\colon X \to \boxtimes X$ exist.
  Every closed term $M_0$ corresponds to a trackable map $\widehat{M_0} \defeq (\mathsf{c}_{M_0}, M_0, \mathfrak{M}_0) \colon \top \to \Lambda$ where $\mathsf{c}_{M_0}$ is the constant function with the value $M_0$ and $\mathfrak{M}_0$ is propositionally equal to the constant function with the value $\refl_{\twoheadrightarrow} : M_0 \reduce M_0$.

  Consider~\eqref{eq:naturality-quoting}  when $f = \widehat{M_0}$ and $M_0 = \mathtt{I}$ and $\mathtt{I I}$.
  Note that $\boxtimes \widehat{M_0}$ maps an arbitrary inhabitant of $\boxtimes \top$ to $(M_0, M_0, \refl_{\twoheadrightarrow})$.
  Hence, by assumption $\bproj{\eta_\Lambda}\,\mathtt{I} = (N , \mathtt{I}, r) \geq (\mathtt{I}, \mathtt{I}, \refl_{\twoheadrightarrow})$ so that $\mathtt{I} \reduce N$ and thus $N =_\alpha \mathtt{I}$. 
  On the other hand, $\bproj{\eta_\Lambda}\,(\mathtt{I}\,\mathtt{I}) \geq (\mathtt{I\,I}, \mathtt{I\,I}, \refl_{\twoheadrightarrow})$ by assumption, so $\bproj{\eta_\Lambda}\,(\mathtt{I}\,\mathtt{I}) \equiv (N, \mathtt{I\,I}, s)$ for some $N$ with $\mathtt{I\,I} \reduce N$ but also $s : N \reduce \mathtt{I\,I}$.
  Thus, $N$ can only be $\mathtt{I\,I}$. 
  \ltnote{this is not a very type-theoretic argument}

  Now let the tracker of $\eta$ be $\mathtt{Q}$.
  It follows that $\mathtt{Q}\,\mathtt{I} \reduce \qquote{I}$ but also $\mathtt{Q}\,\mathtt{I\,I} \reduce \qquote{\mathtt{I\,I}}$, leading to a contraction.
\end{proof}

It remains unclear to us if the existence of a natural transformation $\mathcal{I} \to \boxtimes_K$ can be refuted.
...
%  For every $A : \mathord{\laterp}\,\UU$ if $\laterp (\alpha : \mathbb{T}).\,\mathsf{isProp}(A[\alpha])$ then $\laterp(\alpha : \mathbb{T}).\,A[\alpha]$ is also a proposition. 
%\end{lemma}
...
\begin{theorem}\label{thm:GL-no-eval}
  There is no family of trackable maps $\Box^\kappa X$ to $X$ indexed by assemblies~$X$ for any clock $\kappa$.
\end{theorem}
\begin{proof}
  Assume such a family $\epsilon_X : \Box^\kappa X \to X$ exists.
  In particular, consider $\epsilon_\bot \colon \Box^\kappa \bot \to \bot$. 
...
\begin{proof}
  ...
  \end{enumerate}
  Therefore $(F[\qquote{\mathsf{fix}\,F}], x_0, r_0)$ is an inhabitant of $\Box^\kappa X$ which evidently is realised by $\qquote{F[\qquote{\mathsf{fix}\,F}]}$ and thus by \Cref{lem:global-element} it gives rise to a global element~$f^\dagger$ of $\Box X$, concluding our proof.
\end{proof}

We may wish to prove further that the global element $f^\dagger$ is indeed a fixpoint of $f$ in the sense that $f^\dagger \sim \Box(f \circ f^\dagger) \circ \bigstar$ in line with Kavvos' definition~\cite[Definition~5.29]{Kavvos2020} where $\bigstar : \top \to \Box \top$ is a global element of $\Box \top$ given by $\star : \Unit$ and its realiser $\lambda \mathtt{x}.\,\mathtt{x}$ using~\Cref{lem:global-element}.
...
This situation echoes the discussion of~\Cref{remark:difference-boxtimes}.

\begin{proposition}
  \ltnote{show that LHS and RHS have the same extension}
\end{proposition}
...
The development of the \SFour modality $\boxtimes$ refines that of Kavvos' $\boxtimes_K$ by \Cref{thm:S4-no-quoting} and as discussed in \Cref{remark:difference-boxtimes}, so the first part of results here are not just replicates of the set-theoretic results in homotopy type theory. 
...
\ltnote{The above remark suggests a theory on categories enriched not by $\PP$-sets but \emph{partially ordered $\PP$-sets}, which is beyond the scope of this paper, so we refrain ourselves from further discussion.}
