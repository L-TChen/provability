...
For any type~$A$, the \emph{truncation} of~$A$ is a type~$\brck A$ for which there is an introduction rule that wraps any $a : A$ into $|a| : \brck A$, and a higher one that introduces an inhabitant of $|a| =_{\brck A} |b|$ for any $a$,~$b : A$, equating all inhabitants of~$\brck A$ and making~$\brck A$ a proposition; the elimination rule maps $\brck A$ to a proposition~$P$ provided that $A \to P$.
...
This \GL modality then partially answers the question following Theorem~5.38 in the work of Kavvos~\cite{Kavvos2020} as one of the main contribution of this paper.
