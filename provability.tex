...
However, when $\bproj{X} = \Lambda_0 \to \Lambda_0$, an inhabitant of type~\eqref{eq:alternative-right-totality} amounts to saying that the quoting function is $\lambda$-definable, contradicting~\Cref{prop:no-quoting} if $\Lambda_0$ is equipped with an assembly structure as given in~\Cref{ex:assembly-lambda} below.
On the other hand, an instance of~\eqref{eq:right-total} amounts to the Church's thesis which is independent of univalent type theories~\cite{Swan2019}.
