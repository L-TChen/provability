...
\begin{itemize}
    \item typed metaprogramming and staged computation
    \item many modal type systems: temporal, \citet{Davies2001b}, contextual, \citet{Kavvos2017a}
    \item practical implementations: MetaML, MetaOCaml, (Typed) Template Haskell, BER MetaOCaml
\end{itemize}

\begin{itemize}
    \item \SFour; a natural requirement (and can be taken as a default), for example there is a $\lambda$-term that evaluates a Gödel code \cite[Theorem~8.1.6]{Barendregt1984a}
    \item a `new' construct is \citet{Kavvos2017a}'s intensional recursion (cf.\ extensional/general recursion, which is logically inconsistent, but intensional recursion isn't), which can be used as a form of recursion which allows the access to its own code (intension), and has the potential to enable reflective programming etc (quote Kavvos if necessary)---if we can construct a value (of type~$A$) from its own code (of type~$\Box A$), then intensional recursion gives us that value (of type~$A$). Computationally, this is Kleene's Second Recursion Theorem~\cite[Theorem~6.5.9]{Barendregt1984a}.
    \item \citet{Kavvos2017a} merges \SFour and \GL, but that leads to logical inconsistency (because\ldots), makes it impossible to write internal proofs
\end{itemize}
...
\begin{itemize}
    ...
    \item there has been less work on denotational semantics: (Gabbay\&Nanevski, 2013) and \citet{Kavvos2017b} on \SFour, and they didn't discuss \GL
    ...
    \begin{itemize}
        ...
        \item In the case where the result of executing a piece of code can be immediately used in the same stage, that is, if there is an operation $\Box A \to A$, then the system satisfies the \SFour modal logic.
...
\begin{itemize}
    \item The basis of our investigation is \citet{Kavvos2017b}'s $\PP$-categories, where morphisms are endowed with two kinds of equality, which we can use to model intensional and extensional equalities.
    ...
    It is still not possible for $X \to \Box X$ to be natural in~$X$ for the same reason as the case of $\boxtimes$, whereas $\Box X \to X$ can no longer be validated by projection (because the value from $(M, x)$ is only available later, not now), and in fact will be shown to be impossible.
...
\begin{itemize}
    \item We develop our theory in homotopy type theory (HoTT) mainly for convenience.
    
    \item HoTT provides a high level of granularity of expressiveness clarifying the difference between property versus structure that we need for defining various notions above. 
    
    \item Also, we can work with a more structured version of sets within HoTT; in particular, equality between set elements can be handled much more naturally and conveniently, unlike in other dependent type theories.
    
    \item Due to the advances of formalised mathematics, our arguments can be fully formalised and checked by the proof assistant Agda with the aid of the community contribution (the cubical library, PLFA, etc.).
...
Compare with \citet{Kavvos2017a} in terms of the degree of intensionality
