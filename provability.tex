...
However, let naturals be realised by terms reducible to Church numerals (\Cref{ex:assembly-naturals} below) and functions be realised in the typical way.
By function extensionality, any inhabitant~$G$ of~\eqref{eq:alternative-right-totality} for $\bproj{X} = \Nat \to \Nat$ would then allow us to decide whether a function~$f$ is propositionally equal to a constant function or not by comparing their realisers $Gf$ and~$G(\lambda x.\, 0)$ as shown by Troelstra~\cite{Troelstra1977,forster2021}, which is not possible.
Hence, the type~\eqref{eq:alternative-right-totality} is just empty. 
...
Hitherto, we have reformulated and refined the previous results (based on a partial combinatory algebra, though) of the \SFour modality by Kavvos~\cite{Kavvos2017b} in HoTT.
