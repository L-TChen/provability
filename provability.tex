...
Note that this is not just a direct translation from its set-theoretic formulation to homotopy type theory.
...
\begin{definition}\label{def:assembly}
  ...
  \begin{romanenumerate}
    \item $\mathsf{isRightTotal}(\Vdash_X)$ is a mere proposition defined as $\forall (x : |X|).\, \exists (M : \Lambda_0).\, M \Vdash x$, and
...
First, we do not require the type $M \Vdash_X x$ to be a mere proposition (contrary to its topos-theoretic formulation~\cite{Birkedal2002a}).
...
\begin{definition}\label{def:trackable}
  Given assemblies $X$ and $Y$, a \emph{trackable map} from $X$ to $Y$ consists of a function $\bproj{f}$ from $|X|$ to $|Y|$ and a term $F : \Lambda_1$ with a free variable $\mathtt{x}$ such that $F[M/\mathtt{x}] \Vdash \bproj{f}\,x$ whenever $M \Vdash x$.
...
To untie the knot but to retain the interpretation of $\Box A$ as $A$-typed code, we refine our interpretation with the notion of computation stages. 
...
\]
where $\rhd (\alpha : \mathbb{T}) .\, \brck{M \Vdash_X x [\alpha]}$ is still a mere proposition by~\Cref{lem:proposition-closed-under-later} and the realisability relation is defined as the same as the comonadic exposure~$\boxtimes$.
The only difference between $\bproj{\Box X}$ and $\bproj{\boxtimes X}$ is the extensional part which becomes $\rhd\bproj{X}$ so that the extension of $(M, x, \star)$ is available after a time step. 
...
\begin{proof}
  ...
  We will show that there exists $\mathsf{bang}\colon \rhd \Empty \to \Empty$ so that by guarded recursion $\Empty$ is derivable.
  
  First, let $x$ be an inhabitant of $\rhd \Empty$. We aim at constructing an inhabitant of
  \[
    \Box \bot \equiv \sum_{M : \Lambda_0} \sum_{x : {\rhd \Empty}} \rhd (\alpha : \mathbb{T}) .\, \brck{M \Vdash_\bot x [\alpha]}
  \]
  so that the function $\bproj{\epsilon_\bot}$ from $\bproj{\Box \bot}$ to $\bproj{\bot} \equiv \Empty$ can be applied to derive $\Empty$.
  For arbitrary closed term $M$, say $\mathtt{\lambda x.\, x}$, we can apply the recursion principle~$\mathsf{rec}_{\Empty}$ of the empty type to~$x$ after a time step $\alpha$, so 
  \[
