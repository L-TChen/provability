...
\begin{example} \label{ex:global-element-of-unit}
  A global $\PP$-element $\bigstar\colon \top \to \boxtimes \top$ corresponding to the designated inhabitant~$\star : \Unit$ is given by $(\mathtt{I}, \star, \refl_\twoheadrightarrow)$ with a realiser $\qquote{\mathtt{I}}$ using \Cref{lem:global-element}. 
  ...
  \hsnote{So not $\PP$-functorial?}
  Furthermore, the tracker of a global element might not even be a closed term but an open term.%
  \hsnote{So?}
\end{example}
\ltnote{say somewhere that $\boxtimes$ is not a $\PP$-functor.}
...
\hsnote{Why not a theorem? Why mention the global elements?}
\[
...
What \Cref{re:boxtimes-is-not} implies is that the ordinary naturality is not always the right notion in the presence of intensionality, since the intension part of $\boxtimes f$ is applied verbatim.%
