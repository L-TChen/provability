...
In this section, we first introduce an exposure $\boxtimes\colon \Asm(\Lambda) \looparrowright \Asm(\Lambda)$ which models the \SFour modality and show that a generic quoting function $X \to \boxtimes X$ cannot exist.%
\hsnote{revise}
...
The construction refutes not only $X \to \Box X$ by the same argument for $\boxtimes$ but also the reflection principle $\Box X \to X$.
...
\hsnote{emphasise code unrolling in stages}
...
\hsnote{What's the role of this proposition?}
...
\hsnote{Don't mention naturality?}
...
\begin{proof}
  ...
  \end{enumerate}
  Evidently $(\mathsf{fix}\,F, x_0, r_0')$ is realised by $\qquote{\mathsf{fix}\,F}$, and thus by \Cref{lem:global-element} it gives rise to a global element~$f^\dagger$ of $\Box X$.
...
Finally, it is not clear how to refute the existence of internal quoting for $\boxtimes_K$.
By taking $\lambda$-terms subject to $\alpha$-equivalence as realisers and $\qquote{M}$ as realisers for $(M, x, r) : \bproj{\boxtimes X}$, the exposure~$\boxtimes$ is able to distinguish computationally equivalent intensions, and thus $\boxtimes$ being more intensional than~$\boxtimes_K$ lacks well-behaved extensional properties.

Artemov and Beklemishev~\cite{Artemov2005} pointed out that
Gödel attempted to use classical \SFour modal logic to model provability for Peano Arithmetic ($\mathbf{PA}$) but realised that $\mathsf{Prov}(A) \defeq \exists x.\, \mathsf{Proof}(x, A)$ cannot be \SFour.
Löb found the well-known \GL axiom for provability and
Solovay showed that \GL is complete for~$\mathbf{PA}$.
