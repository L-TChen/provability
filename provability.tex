...
\begin{definition} \label{ex:global-element-of-unit}
  A global $\PP$-element $\bigstar\colon \top \to \boxtimes \top$ corresponding to the designated inhabitant~$\star : \Unit$ is given by $(\mathtt{I}, \star, \refl_\twoheadrightarrow)$ with a realiser $\qquote{\mathtt{I}}$ by \Cref{lem:global-element}. 
\end{definition}
...
\ltnote{insert an explanation of the following proposition}
\hsnote{Why mention the global elements?}

Given a global $\PP$-element~$a$ of~$X$, we define its \emph{quotation} as the element $\boxtimes a \circ \bigstar$ of~$\boxtimes X$.
We then say that a trackable map $q\colon X \to \boxtimes X$ \emph{quotes} an element $a$ if $q \circ a$ is extensionally equal to~$\boxtimes a \circ \bigstar$.
The (\textbf{4}) axiom then can be realised by a family of trackable maps which actually quote `well-behaved' global $\PP$-elements:
...
Moreover, $\delta_X$ quotes any element $a$ of~$\boxtimes X$ given by~\Cref{lem:global-element}.
%  \[
...
%  \]
\end{proposition}
The map $\delta_X$ may fail to quote if the element $a$ is not given by \Cref{lem:global-element}, since in general the intension part of $\boxtimes a$ is applied only verbatim.
That is, $\bproj{\boxtimes a}(\mathtt{I}, \star, \refl_\twoheadrightarrow)$ is $ (F[\mathtt{I}], (M, x, r), s)$ where $F[\mathtt{I}]$ is not necessarily equal to~$\qquote{M}$.
...
Instead, we may consider an order induced by the reduction relation between inhabitants of $\boxtimes X$: define $(M, x, r) \leq (N, y, s)$ if $M \reduce N$ and $x = y$, and
accordingly $f \leq g$ if $\bproj{f}\,x \leq \bproj{g}\,x$ for all~$x$.
Then we only have $\bproj{{\boxtimes} {\boxtimes} f \circ \delta_X} \leq \bproj{\delta_Y \circ {\boxtimes} f}$ instead.
\end{remark}

In short, the naturality appears inappropriate for a family of trackable maps $\tau_X$ to hold in general.
However any family $\eta$ of trackable maps to realise $A \to \boxtimes A$ should at least makes
\[
  \xymatrix{
    \top \ar[d]_{a} \ar[r]^{\eta_\top} & \boxtimes \top \ar[d]^{\boxtimes a} \\
    \Lambda \ar[r]_{\eta_\Lambda} & \boxtimes \Lambda
  }
\]
commute for elements $a$ given by \Cref{lem:global-element} as in \Cref{prop:four-rule}.
Yet, this limited form of naturality is still impossible to achieve.
The key is the following lemma which relies on~\Cref{prop:no-quoting}. 

\hsnote{Is there more to say about the importance of lax naturality? Just how important is naturality (since we seem to imply naturality is essential for $\eta$)?}
...
\begin{theorem}\label{thm:S4-no-quoting}
  There are no trackable maps $\eta_\Lambda\colon \Lambda \to \boxtimes \Lambda$ and $\eta_\top\colon \top \to \boxtimes \top$ satisfying $\eta_\Lambda \circ a \sim \boxtimes a \circ \eta_\top$ for global $\PP$-elements~$a$ of $\Lambda$ given by \Cref{lem:global-element}.
