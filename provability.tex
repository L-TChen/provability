...
\keywords{Löb's theorem, guarded recursion, realizability, cubical type theory, modal type theory, metaprogramming}
...
\supplement{The results based on the ticked cubical type theory were formalised in Guarded Cubical Agda with source code available at \url{https://github.com/L-TChen/provability}.}
...
\acknowledgements{We want to thank Alex Kavvos and Martin Escard\'o.}
...
\begin{abstract}
  ...
  Of the utmost significance of our understanding for $\Box A$ is the insight that its denotation should involve not only extensions but also intensions, e.g., derivations of a proof, code of programs, or realisers, inspired by its provability reading.
  Our results are established in two styles.
  One is the `propositions-as-type' approach using ticked cubical type theory---an extension of cubical type theory with guarded recursion.
  The other is `logic over a type theory' using the internal logic of topos extended with a later modality on the level of types and on the level of propositions respectively. 
  By viewing~$\Box A$ as a type of typed code in the sense of Davies and Pfenning, our work can be viewed as a step towards typed metaprogramming with non-structural but guarded recursion.
\end{abstract}
