...
\begin{abstract}
  ...
  We consider the ordinary category $\Asm(\Lambda)$ of assemblies on untyped lambda calculus and \emph{functions with trackers} in contrast to the $\PER$-category $\ASM(\mathcal{A})$ of assemblies on an arbitrary partial combinatory algebra $\mathcal{A}$ proposed by Kavvos.
  Then, we construct an endofunctor as the interpretation of the (provability) modality~$\Box$.
  Of the utmost significance of our understanding for $\Box A$ is that its denotation should involve not only extensions but also intensions, e.g., derivations of a proof, code of programs, or $\lambda$-terms modulo \emph{$\alpha$-equivalence only}, inspired by its provability reading.
...
\section{Category of assemblies over untyped lambda calculus}
