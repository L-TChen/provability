...
To sum up, we have just showed that $\ASM(\Lambda)$ has the initial object, finite products, and exponentials.
That is, 
\begin{corollary}
  $\ASM(\Lambda)$ is a cartesian closed $\PP$-category.
\end{corollary}
...
In this section, we first introduce a comonadic exposure $\boxtimes\colon \ASM(\Lambda) \looparrowright \ASM(\Lambda)$ of the $\PP$-category of assemblies which is different from the one defined by Kavvos~\cite[Section~9.1]{Kavvos2017b}.
For example, the one by Kavvos preserves finite products including the terminal object~$\top$, but ours does not with good reason. 
Most importantly, we are able to refute the completeness principle $A \to \boxtimes A$, understood as a quoting function from the computational perspective, while we did not find a way to refute using Kavvos' definition.
Hitherto, we complete our cook's tour of the set-theoretic results based on a partial combinatory algebra in terms of homotopy type theory.

Thereafter, we move to a variant of guarded type theories---Ticked Cubical Type Theroy~\cite{Mogelberg2019a}---and introduce an exposure-like construction $\Box$ to model Gödel-Löb axiom.
...
In the end, we prove that the deductive version of Gödel-Löb axiom, i.e.\ given a trackable map $f : \Box X \to X$ one is able to construct $\Box X$ using Nakano's guarded recursion on extensions and the Kleene's second recursion theorem on intensions.
Then, the main result of this paper---the internalised version of Gödel-Löb axiom---follows routinely.


\paragraph*{Comonadic exposure for $S4$ modality}
As an assembly $X$ records the interplay between intensions, i.e.\ untyped $\lambda$-terms, and extensions $x : \barred{X}$, it is now possible to bring out intension to the extensional level by pairing an inhabitant $x : \barred{X}$ with its realiser, i.e.\ $\barred{\boxtimes X} \defeq \sum_{M : \Lambda_0} \sum_{x : \barred{X}} \brck{ M \Vdash_X x }$
as the carrier of $\boxtimes X$.
The realisability relation is then defined by
\[
  N \Vdash_{\boxtimes X} (M, x, \star) \defeq N \reduce \qquote{M}
\]
where $\qquote{M}$ is the Gödel numbering of $M$. 
To make the pair $(\barred{\boxtimes X}, \Vdash_{\boxtimes X})$ an assembly, it remains to give $r_{\boxtimes X}$ and $t_{\boxtimes X}$.
The inhabitant $r_{\boxtimes X}$ is just given by the transitivity of $\reduce$.
Also each $(M, x, \star)$ clearly has a realiser $\qquote{M}$.
In short, $\boxtimes X \defeq (\barred{\boxtimes X}, \Vdash_{\boxtimes X}, r_{\boxtimes X}, t_{\boxtimes X})$ is indeed an assembly.

As for the mapping on morphisms, consider a trackable map $(f, F, \mathfrak{f})$ from $X$ to $Y$.
By the definition of a trackable map, we have $F[M] \Vdash_Y f\,x$ whenever $M \Vdash x$.
Therefore, 
\[
  \boxtimes f : (M, x, \star) \mapsto (F[M], f\,x, \star')
\]
defines a function from $\barred{\boxtimes X}$ to $\barred{\boxtimes Y}$ where $\star'$ is given by the recursion principle of the propositional truncation.
It remains to give a tracker of $\boxtimes f$ such that $\boxtimes F[N] \reduce \qquote{F[M]}$ whenever $N \reduce \qquote{M}$. 
At this point, recall from \Cref{sec:preliminaries} that we have a term $\mathtt{subst} : \Lambda_0$ such that $\mathtt{subst}_x\,\qquote{F}\,\qquote{M} \reduce \qquote{F[M/\mathtt{x}]}$. 
Hence, by defining $\boxtimes F$ as $\mathtt{subst}_{\mathtt{x}}\,\qquote{F}\,\mathtt{x}$, it follows that
\[
  \left(\mathtt{subst} \,\qquote{F}\,\mathtt{x}\right)[N/\mathtt{x}]
  \equiv 
  \mathtt{subst}\,\qquote{F}\,N
  \reduce 
  \mathtt{subst}\,\qquote{F}\,\qquote{M}
  \reduce \qquote{F[M/\mathtt{x}]},
\]
as desired, which completes the definition of $\boxtimes\mathfrak{f} : \prod_{M : \Lambda_0} \prod_{x : \barred{X}}
\left(N \Vdash_{\boxtimes X} x \to \boxtimes F[N]\Vdash_{\boxtimes Y} \boxtimes f\, x\right)$.
Therefore, $\boxtimes(f, F, \mathfrak{f})$ defined by $(\boxtimes f, \boxtimes F, \boxtimes \mathfrak{f})$ is indeed a trackable map.
...
\begin{theorem}
  There is no trackable map $q_A : A \to \Box A$ natural in $A$.
\end{theorem}
