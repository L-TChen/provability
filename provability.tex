...
The above example also shows that $\boxtimes \top$ cannot be $\PP$-isomorphic to $\top$, since there are countably many representations for $\star : \Unit$, i.e.\ inhabitants of $\bproj{\boxtimes \top}$ while there is exactly one inhabitant of $\bproj{\top} \equiv \Unit$. 
...
In addition, there is a family of functions $\bproj{\delta_X}\colon \boxtimes X \to \boxtimes \boxtimes X$ defined by $(M, x, r) \mapsto (\qquote{M}, (M, x, r), \refl_\twoheadrightarrow)$ and
tracked by~$\mathtt{quote}$ corresponding to the quotation of a quoted term. 
For any global element $a\colon \top \to \boxtimes X$ given by~\Cref{lem:global-element} whose tracker is indeed the quoted term~$\qquote{M}$, we can see that $\delta_X \circ a$ coincides with $\boxtimes a \circ \bigstar\colon \top \to \boxtimes \boxtimes X$
\[
...
What \Cref{re:boxtimes-is-not} implies is that the ordinary naturality is not always the right notion in the presence of intensionality, since the intensional part of $\boxtimes f$ is only applied verbatim.
...
\end{lemma}
\begin{proof}
  Suppose that $\eta_\Lambda$ is tracked by a term~$\mathtt{Q}$.
  Being a tracker means that $\mathtt{Q}\,N \Vdash (M, M, \refl_{\twoheadrightarrow})$ which is judgementally equal to $\mathtt{Q}\,N\reduce \qquote{M}$ whenever $N \Vdash M$ (judgementally equal to $N \reduce M$).
  It follows that $\mathtt{Q}\,M \reduce \qquote{M}$ and thus such $\mathtt{Q}$ cannot exist.
\end{proof}
...
\begin{proof}
  ...
  To prove the statement, it suffices to show that $\eta_\Lambda\,M_0$ is propositionally equal to $(M_0, M_0, \refl_{\twoheadrightarrow})$ so that this family $\eta$ cannot exist by \Cref{lem:no-quoting} and functional extensionality.
  ...
  Note that $\bproj{\boxtimes\widehat{M_0}}(M, x, r) = (M_0[M], M, \refl_\twoheadrightarrow)$ by definition for any inhabitant $(M, x, r)$ of type $\boxtimes \top$ where $M_0[M]$ is propositionally equal to $M_0$.
...
\ltnote{\Cref{re:boxtimes-is-not} suggests a theory on categories enriched not by $\PP$-sets but \emph{partially ordered $\PP$-sets}, which is beyond the scope of this paper, so we refrain ourselves from further discussion.}
