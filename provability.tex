\documentclass[draft,a4paper,UKenglish,numberwithinsect,cleveref,thm-restate]{lipics-v2021}
...
Kavvos~\cite{Kavvos2017b} advocates that the provability modality~$\Box$ and the Gödel-Löb axiom $\Box(\Box A \to A) \to \Box A$ can also be understood as the type of code of type~$A$ and as intensional recursion respectively from the computational perspective.
Since we already have an exposure~$\boxtimes$ modelling typed code, a natural approach is to extend~$\boxtimes$ to model the Gödel-Löb axiom in addition to \SFour.
However, it is known that the Gödel-Löb axiom is incompatible with the reflection principle $\Box A \to A$.
...
Therefore, by \Cref{prop:strict-initial,thm:S4-exposure}, we cannot expect the exposure $\boxtimes$ to model the Gödel-Löb axiom if we want the type system to be logically consistent.

To untie the knot but to retain consistency and the understanding of $\Box A$ as code of type~$A$, we observe that in general $\Box A$ and $\boxtimes A$ are types for different kinds of code: code constructed with intensional recursion can only be gradually expanded in stages (or otherwise may result in non-termination), whereas code supporting \SFour is only a special case where next stages include the current one so that $\epsilon_X\colon {\boxtimes} X \to X$ is allowed.
Therefore, separately from~$\boxtimes$, we give an exposure $\Box \colon \Asm(\Lambda) \looparrowright \Asm(\Lambda)$ modelling \GL in a staged setting (\Cref{sec:GL}).
