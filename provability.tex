...
% 
\usepackage[noinline,margin]{fixme}
...
\keywords{provability, guarded recursion, realisability, modal type, metaprogramming}
...
The choice of using~$\qquote{M}$ (rather than~$M$ as chosen by Kavvos~\cite{Kavvos2017b}) as the intensions of $(M, x)$ prevents $X \to \boxtimes X$ from having any computational meaning, that is, there is no generic internal quoting operation.
\ltnote{Ask Kavvos}
...
Hence, by refuting both congruence and internal quoting, which previous work~\cite{Gabbay2013,Kavvos2017b} fails to achieve, our denotational semantics is more intensional such that it provides a finer-grained equality which allows us to distinguish computationally equivalent intensions.%
...
\hsnote{need to give a bit of detail for the exponential construction to make sense}
...
\begin{lemma}\label{lem:global-element}
  ...
  \begin{enumerate}
    ...
    \item Every pair of $x : \bproj{X}$ and $M : \Lambda_0$ with $r : M \Vdash_X x$ defines a element of $X$, i.e.\ a trackable map $(\lambda\_.\, x, M, \lambda\_.\,\lambda\_.\, \lambda\_.\, r)$ from $\top$ to $X$.
  \end{enumerate}
\end{lemma}
...
Even further, we can restrict to \emph{closed} elements, which are elements given by~\Cref{lem:global-element}.
This property relies on the property that the carrier $\bproj{X}$ is a set.
\begin{proposition}
  Two two trackable maps $f, g\colon X \to Y$ are extensionally equal if and only if $f \circ x \sim g \circ x$ for every closed element~$x\colon \top \to X$.
...
\]
To give a tracker of $f^\boxtimes$, recall that there is a term $\mathtt{subst}$ performing term substitution on codes (\Cref{sec:lambda}), and then the term $F^\boxtimes \defeq \mathtt{subst}\,\qquote{F}\,\mathtt{x}$ tracks $f^\boxtimes$ because
\begin{align*}
...
\begin{definition} \label{ex:global-element-of-unit}
  By $\bigstar$ we denote a closed element of~$\boxtimes \top$ given by \Cref{lem:global-element} with $(\mathtt{I}, \star, \refl_\twoheadrightarrow) : \bproj{\boxtimes \top}$ and its realiser $\qquote{\mathtt{I}}$.
\end{definition}
...
\hsnote{So more intensional and better?}
...
\hsnote{Why mention the elements?}
...
We say that a trackable map $q\colon X \to \boxtimes X$ \emph{quotes} an element $a$ of~$X$, thoughts as a reduction $\mathtt{q}[a/x] \mapsto \qquote{a}$ whenever
\[
  \xymatrix{
...
There is a family of functions $\bproj{\delta_X}(M, x, r) \defeq (\qquote{M}, (M, x, r), \refl_\twoheadrightarrow)$ from $\bproj{\boxtimes X}$ to $\bproj{{\boxtimes} {\boxtimes} X}$ tracked by~$\mathtt{quote}$ which quotes closed elements of~$\boxtimes X$.
%  \[
...
Yet, $\delta_X$ may fail to quote if the element $a$ is not closed, since in general the intension part of $\boxtimes a$ is applied only verbatim.
...
\]
in spite that their extensions are the same where $\mathsf{subst}_{\twoheadrightarrow}$ is the witness of the reduction sequence $\mathsf{subst}\,\qquote{F}\,\qquote{M} \reduce \qquote{F[M]}$.
If we define $(M, x, r) \leq (N, y, s)$ whenever $M \reduce N$ and $x = y$ and $f \leq g$ if $\bproj{f}\,x \leq \bproj{g}\,x$ for all~$x$.
Then we only have $\bproj{{\boxtimes} {\boxtimes} f \circ \delta_X} \leq \bproj{\delta_Y \circ {\boxtimes} f}$.
In general, the naturality axiom appears inappropriate in the presence of intensionality.
\end{remark}

The normality condition is realised exactly by~$\mathtt{ap}$ without naturality:
\begin{proposition}
There is a family of trackable maps
from $\boxtimes (X \Rightarrow Y)$ to $\boxtimes X \Rightarrow \boxtimes Y$  tracked by $\lambda x_0.\, \mathtt{ap}[x_1] \cdot x_0$.
\end{proposition}

On the other hand, it is impossible to give a computational meaning of the rule $A \to \boxtimes A$ for arbitrary~$A$.
The key is the following lemma which relies on~\Cref{prop:no-quoting}. 

\hsnote{Just how important is naturality (since we seem to imply naturality is essential for $\eta$)?}

\begin{lemma}\label{lem:no-quoting}
  The function $\eta_\Lambda \colon \Lambda_0 \to \boxtimes \Lambda_0$ given by $M \mapsto (M, M, \refl_\twoheadrightarrow)$ cannot be tracked.
\end{lemma}
%\begin{proof}
...
\begin{theorem}\label{thm:S4-no-quoting}
  There is no trackable map $\eta_\Lambda\colon \Lambda \to \boxtimes \Lambda$ which quotes closed elements of~$\Lambda$.
...
\end{theorem}
\begin{proof}
  Assume $\eta_\Lambda$ with that $\eta_X \circ a \sim \boxtimes a \circ \bigstar$ for any $a \colon \top \to \Lambda$ given by \Cref{lem:global-element}.
  Every closed term $M_0$ defines an element $\widehat{M_0} \defeq (\lambda x.\, M_0, M_0, \lambda N.\, \lambda y.\, \lambda s.\, \refl_\twoheadrightarrow)$ of $\Lambda$ and thus $\bproj{\boxtimes\widehat{M_0}}(N, y, s) = (M_0, M_0, \refl_\twoheadrightarrow)$ for any $(N, y, s) : \boxtimes \top$ by definition.
  By assumption we have
  \[
    \bproj{\eta_{\Lambda}}\,M_0 \equiv \bproj{\eta_{\Lambda}} \left(\bproj{\widehat{M_0}}\,\star\right) =  \bproj{\boxtimes\widehat{M_0}}\,(\bproj{\eta_\top}\,\star) = (M_0, M_0, \refl_\twoheadrightarrow),
  \]
  but \Cref{lem:no-quoting} says that $\eta_\Lambda$ cannot be trackable, a contradiction.
...
The second extension adds a sort $\clock$ with \emph{ticks}~\cite{Bahr2017} to support Nakano's later modality~$\laterp$ and guarded recursion also known as \emph{Löb induction}.
...
This tick~$\diamond$ is needed to type Atkey and McBride's~\cite{Atkey2013b} operator
  ...
  Finally, every function $t : \laterp^\kappa A \to A$ has a delayed fixed point $\mathsf{dfix}\,t : \laterp^\kappa A$ with a path replacing the judgemental fixed point equation (to ensure the termination of reduction semantics), called \emph{guarded recursion} also known as \emph{Löb induction}:
\[
  \begin{minipage}{0.47\linewidth}
    \begin{prooftree}
      \AxiomC{$\Gamma \vdash t : \laterp^\kappa A \to A$}
      \UnaryInfC{$\Gamma \vdash \mathsf{dfix}\,t : \laterp^\kappa A$}
    \end{prooftree}
  \end{minipage}
  \begin{minipage}{0.47\linewidth}
    \begin{prooftree}
      \AxiomC{$\Gamma \vdash t : \laterp^\kappa A \to A$}
      \UnaryInfC{$\Gamma \vdash \mathsf{pfix}^\kappa t : \left(\mathsf{dfix}\,t[\alpha]\right) \equiv_A \left(t(\mathsf{dfix}\,t)\right)$}
    \end{prooftree}
  \end{minipage}
\]
We can further derive the fixed point $t(\mathsf{dfix}\,t) : A$ without any delay.
...
\]
where $\Vdash_{\Box^\kappa X}$ is defined as the same as the exposure~$\boxtimes$.
...
That is, the extension $x$ is available in a next stage but not earlier (within the clock $\kappa$), but the intension $M$ remains the same type.
...
For any trackable map $f$ from $X$ to $Y$, also define $\Box^\kappa f$ as the same as $\boxtimes f$ except that a later modality is involved: 
\[
...
\begin{proposition}
There is a family of trackable maps
from $\boxtimes^\kappa (X \Rightarrow Y)$ to $\boxtimes^\kappa X \Rightarrow \boxtimes^\kappa Y$  tracked by $\lambda x_0.\, \mathtt{ap}[x_1] \cdot x_0$.
\end{proposition}

Similar to~\Cref{thm:S4-no-quoting}, by the non-existence of a term $\mathtt{Q}$ with $\mathtt{Q}\,M \reduce \qquote{M}$ for any arbitrary term $M$, the rule $A \to \Box A$ cannot be given any computational meaning:
\begin{theorem}\label{thm:GL-no-quoting}
  There is no trackable map from $\Lambda$ to~$\Box \Lambda$ which quotes closed elements.
\end{theorem}
...
\begin{proof}
  ...
  First, let $x$ be an inhabitant of $\laterp \Empty$. We construct an inhabitant $(M, x, r)$ of $\Box^\kappa \bot$
%  \[
...
\begin{theorem}[Intensional recursion]\label{thm:GL-recursion}
  For every function $\bproj{f} \colon \bproj{\Box^\kappa X} \to \bproj{X}$ tracked by some $F : \Lambda_1$, there are
  \begin{enumerate}
...
\begin{proof}
  ...
  Applying \Cref{thm:SRT} to $\lambda\mathtt{x}_0.\,F : \Lambda_0$, we obtain a term $\mathsf{fix}\,F : \Lambda_0$ with $\mathsf{fix}\,F \reduce (\lambda\mathtt{x}_0.\,F)\,\qquote{\mathsf{fix}\,F} \onereduce F[\,\qquote{\mathsf{fix}\,F}\,]$.
  ...
  To construct the second element~$f^\ddagger$, we follow the same steps but the third: we conclude $(x_0, r_0) : \sm{x : \bproj{X}} F[\,\qquote{\mathsf{fix}\,F}\,] \Vdash x [\alpha]$ without any delay and thus $x_0 : \bproj{X}$ is realised by $r_0' : \mathsf{fix}\,F \Vdash x_0$.
...
\begin{enumerate}
  \item Compare $\boxtimes$ with $\boxtimes_K$ (\cite{Kavvos2017b}). 
  \item Discuss logic of proofs and provability logic \cite{Beklemishev2006, Goris2009, Shamkanov2014, Shamkanov2016a, Visser2019}.
\end{enumerate}

$\boxtimes$ is subtly different from the comonadic exposure introduced by Kavvos~\cite[Definition~9]{Kavvos2017b}, which we denote by~$\boxtimes_K$ in this paper.
...
Notably, our denotational semantics of the \SFour modality~$\boxtimes$ is more intensional than that of Kavvos' $\boxtimes_K$ because of \Cref{thm:S4-no-quoting} and \Cref{re:difference-boxtimes}.
For the \GL modality~$\Box$, we have given the \emph{first} denotational semantics, which is shown~(\Cref{thm:GL-recursion}) to satisfy the deductive form of the Gödel-Löb axiom---the intensional recursion---using guarded type theory.
