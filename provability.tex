...
\begin{abstract}
  ...
  Of the utmost significance of the interpretation for $\Box A$ is that its denotation involves not only intensions, e.g., derivations of a proof, code of programs, or $\lambda$-terms modulo $\alpha$-equivalence given its provability reading but also their extensions enriched with the notion of computation stages. 
...
In this paper, we use Homotopy Type Theory~\cite{hottbook}, HoTT for short, as the foundation of mathematics. 
The choice of foundations is not a matter of taste but based on a pragmatic reason. 
...
We now briefly review Homotopy Type Theory~\cite{hottbook}, HoTT for short, used in this paper.
...
\subsection{\texorpdfstring{$\PP$}{P}-category theory}
An unusual choice for the purpose of categorical semantics we made is the underlying category theory.
Instead of ordinary categories, we choose to use $\PP$-categories which are categories enriched by $\PP$-sets (also known as partial setoids) and pioneered by \v{C}ubri\'c et al.~\cite{Cubric1998a} to study normalisation in typed $\lambda$-calculus.
Its use is also advocated by Kavvos~\cite{Kavvos2017b} who introduced a concept similar to functor called \emph{exposure} exemplified by the second recursion theorem and more in computability theory.

We now briefly review definitions taken from the works by \v{C}ubri\'c et al.\ and by Kavvos.
\begin{definition}
  A \emph{partial equivalence relation}, \emph{PER} for short, is a symmetric and transitive relation.
  A \emph{$\PP$-set} $(X, \sim)$ is a set $X$ with a partial equivalence relation~$\sim$.
  A \emph{$\PP$-function} from $(X, \sim_X)$ to $(Y, \sim_Y)$ is a function $f\colon X \to Y$ which respects the relation~$\sim$ in the sense that $f\,x \sim f\,y$ whenever $x \sim y$.
\end{definition}
One can then show that the identity function $\id_X : X \to X$ is a $\PP$-function.
The composite of two $\PP$-functions (by function composition) is again a $\PP$-function.
It turns out that $\PP$-sets and $\PP$-functions form a cartesian closed category~$\PP\text{-}\mathbf{Set}$ so that by enriched category theory~\cite{Kelly1982} we have a notion of categories enriched by $\PP$-sets below.

\begin{definition}[{\cite[Definition~2.4]{Cubric1998a}}]
  A $\PP$-category is a category whose hom-sets are $\PP$-sets satisfying associativity and the identity law up to~$\sim$.
\end{definition}
We call a morphism $f$ \emph{well-defined} if $f \sim f$. 

In a $\PP$-category, there are two kinds of equalities for morphisms---one is the underlying identity type $=$ and the partial equivalence relation $\sim$ where the former is considered as the \emph{very intensional equality} and the later as the \emph{extensional equality}. 
This additional equality reflects the fact that, for example, $\alpha$-equivalent terms in $\lambda$-calculus are always $\beta$-equivalent but not the converse.
For categorical semantics, a term is interpreted as a morphism so that an interpretation of $\lambda$-calculus in such a $\PP$-category is possible to discriminate these two kinds of identities, enabling us to model intensionality.

Next we recall the notion of exposures.
In short, it is a functor which does not preserve but \emph{reflect} $\sim$-equality in the following sense.
\begin{definition}
  Given $\PP$-categories $\mathscr{C}$ and $\mathscr{D}$, an \emph{exposure} $Q\colon \mathscr{C} \looparrowright \mathscr{D}$ consists of \emph{(a)} a mapping $Q$ from objects of $\mathscr{C}$ to objects of $\mathscr{D}$ and for every two objects $X$, $Y$ in $\mathscr{C}$ a mapping $Q_{X, Y}$ from $\mathscr{C}(X, Y)$ to $\mathscr{D}(Q X, Q Y)$ such that
  \begin{enumerate}
    \item $Q_{X, X}(\iid_X) \sim \iid_{QX}$,
    \item $Q_{X, Z}(g \circ f) \sim Q_{Y, Z}g \circ Q_{X, Y}f$, and
    \item $f \sim g$ whenever $Q_{X, Y}f \sim Q_{X, Y}g$ for any well-defined morphisms $f, g\colon X \to Y$. 
  \end{enumerate}
\end{definition}
The \emph{identity exposure} $\mathcal{I}$ maps every object to itself as well as morphisms.
Also composing two exposures like functors clearly gives us an exposure. 

Another notion we need is a proper notion of morphisms between exposures below.
\begin{definition}
...
\end{definition}
Similarly, we can define a notion of \emph{comonadic exposures}\footnote{%
  The definition of comonadic exposures $Q$ by Kavvos~\cite{Kavvos2017b} requires additionally that $Q$ is cartesian and product-product preserving.
  We will revisit this issue in~\Cref{sec:provability}.
}
as an exposure $Q$ with natural transformations $\epsilon\colon Q \to \mathcal{I}$ and $\delta\colon Q \to Q \circ Q$ satisfying the co-associative law and the co-unit laws up to~$\sim$.

Finally, note that even though we choose HoTT as the foundation, we do not require the carrier of a $\PP$-set to be a set ($0$-type) and the relation $\sim$ proof-irrelevant ($-1$-type). 
This is the only main difference between the set-theoretic formulation and the type-theoretic one for $\PP$-categories.
...
We say that $a$ \emph{realises} $x$ or $a$ is a \emph{realiser} of $x$ if $n \Vdash x$.
...
As a result, the interaction with reduction $\reduce$ is formulated as an additional property of~$\Vdash$ in line with the definition on an ordered PCA~\cite[Section~2.3]{Hofstra2003}.
...
Given any $r : M \Vdash_X x$, the inhabitant $\mathfrak{g}(\mathfrak{f}\,r)$ is of type $G[F[M]]$ and its transportation along $p$ has type $G[F][M]$.
Thus, we can define an operation $\lambda M.\,\lambda x.\,\lambda r.\, \mathsf{transport}(p, \mathfrak{g}(\mathfrak{f}\,r))$.
...
\begin{proposition}
  Every pair of trackable maps $f_1, f_2 : X \to Y$ with the carrier $\bproj{Y}$ a set are extensionally equal if $f_1 \circ x \sim f_2 \circ x$ for every global element $x$ to $X$. 
\end{proposition}
...
\subsection{A recap: Gödel encoding and Second Recursion Theorem}
Before entering into the main topic of this section, we review the Second Recursion Theorem in $\lambda$-calculus briefly. The details can be found in the classic textbook on $\lambda$-lambda calculus by Barendregt~\cite[Section~6.5]{Barendregt1984a}.

In $\lambda$-calculus, we can assume that there is a function between $\lambda$-terms
\[
  \qquote{\cdot} : \Lambda \to \Lambda_0
\]
mapping any term $M$ to a closed term $\qquote{M}$ such that $\qquote{M}$ is normal,
$M \equiv_\alpha N$ whenever $\qquote{M} \equiv_\alpha \qquote{N}$, and moreover there are open terms $\mathtt{ap}, \mathtt{subst}, \mathtt{quote}, \mathtt{eval}$ satisfying
...
\end{align*}
We call the function $\qquote{\cdot}$ a \emph{Gödel encoding} of $\lambda$-terms.
The quoted term $\qquote{M}$ is often called a Gödel \emph{number} since the encoding function $\qquote{\cdot}$ is typically defined by assigning a natural number $\#M$ represented as a Church numeral $\mathbf{c}_{\#M}$ to every term~$M$, a method originally invented by Gödel to prove Incompleteness Theorems.
...
For more details on the axiomatic characterisation of encoding, see Polonsky~\cite{Polonsky2011}.
...
Otherwise, assume we have a term $\mathtt{Q}$ such that $\mathtt{Q}\,M \reduce \qquote{M}$ for arbitrary $M$. Then, consider $\mathtt{I} \defeq \mathtt{\lambda x.\, x}$ and $\mathtt{I}\,\mathtt{I}$. 
We have $\mathtt{Q}\,(\mathtt{I}\,\mathtt{I}) \reduce \qquote{\mathtt{I}\,\mathtt{I}}$ by assumption but also $\mathtt{Q}\,(\mathtt{I}\,\mathtt{I}) \reduce \mathtt{Q}\,\mathtt{I}\reduce \qquote{I}$ by the congruence rule. 
Therefore, by confluence and the assumption that $\qquote{M}$ is normal, it follows that $\qquote{I\,I} \equiv_\alpha \qquote{I}$ and thus $\mathtt{I\,I} \equiv_\alpha \mathtt{I}$, a contradiction.

Apart from the well-known Kleene's (First) Recursion Theorem, a similar statement also works for quoted terms as follows.
\begin{theorem}[Second Recursion Theorem]
  Every term $F$ in untyped $\lambda$-calculus has a term $M$ with $M \reduce F\,\qquote{M}$.
\end{theorem}

If $F$ is in the form of abstraction, the above theorem boils down to the following case which will be used later.
...
\begin{proof}[Proof sketch]
  ...
  For the families of maps $\boxtimes A \to A$ and $\boxtimes A \to \boxtimes \boxtimes A$ natural in $A$, recall that there are terms $\mathtt{eval}$ and $\mathtt{quote}$ which evaluates a code to its representing term and computes the code $\qquote{N}$ of a given term~$N \equiv \qquote{M}$ in the form of code respectively. That is, 
  \[
    \mathtt{eval}\,\qquote{M} \reduce M
    \quad\text{and}\quad
    \mathtt{quote}\,\qquote{M} \reduce \qquote{\qquote{M}}.
  \]
Use these terms as trackers for $\boxtimes A \to A$ and $\boxtimes A \to \boxtimes \boxtimes A$ respectively.
