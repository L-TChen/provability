...
\emph{Metaprogramming} is about writing programs that manipulate programs.
A particular form of metaprogramming is \emph{staged computation}, where fragments of a program are marked to be evaluated in multiple stages, so that the program can be partially evaluated to produce a more efficient version.
There have been investigations into type-theoretic foundations for metaprogramming and particularly the concept of staged computation modelled with temporal modality, \SFour modality~\cite{Davies2001b}, contextual \SFour modality, \GL modality~\cite{Kavvos2017a}, and so on, which have influenced the design of practical metaprogramming systems to varying degrees.\ltnote{cite metaprogramming languages}

Let $\mathtt{Code}\,A$ denote the type of code that evaluates to values of type~$A$ in the next stage.
...
the (\textbf{T}) axiom $\boxtimes A \to A$ corresponds to the evaluation of code to its value in the same stage.
...
Recently, Kavvos proposed a `new' construct called \emph{intensional recursion}~\cite{Kavvos2017a} for $\mathtt{Code}$ that can be used to construct a value recursively from its own code, extending the capabilities of metaprogramming.
In contrast to general recursion $(A \to A) \to A$, which cannot inspect its own code (intension) but only its value (extension), intensional recursion alone does not lead to logical inconsistency.
Intensional recursion amounts to the $(\mathbf{GL})$ axiom $\Box (\Box A \to A) \to \Box A$ in provability logic where $\Box$ stands for `provability', and its computational behaviour mirrors Kleene's second recursion theorem.
Kavvos explored the computational capabilities of a variant of PCF extended with $\mathtt{Code}\,A$ subject to both (\SFour)~and~(\GL);
unfortunately, when $\mathtt{Code}\,A$ is understood as \emph{the} type of code merging~(\SFour) and~(\GL), a contradiction is derivable, making it impossible to treat the type system as a logical foundation.

\begin{itemize}
    \item In our approach, we want that \SFour and \GL coexist in a single system but are kept as separate modalities $\boxtimes$~and~$\Box$ to maintain logical consistency.
    \item start with a denotational semantics
    \item there has been less work on denotational semantics: (Gabbay\&Nanevski, 2013) and \cite{Kavvos2017b} on \SFour, and they didn't discuss \GL
    \item the key is to find out the commonalities and differences between the two modalities, for which we take inspiration from staged computation
    \begin{itemize}
        \item Executing a piece of code can result in another piece of code to be executed, and these consecutive executions are abstractly referred to as computation stages, whose structure is a form of possible worlds and can be reasoned about by suitable modal logics.
        \item In the special case where the result of executing a piece of code can be immediately used in the same stage, that is, if there is an operation $\Box A \to A$, then the system satisfies the \SFour modal logic.
        \item need stage distinction for \GL; inconsistent otherwise
    \end{itemize}
    \item in short, we observe that a term of type $\Box A$ is, in general, code to be evaluated in a later stage, whereas $\boxtimes A$ is the type of code that can be evaluated in the current stage; this is the intuition behind the design of our denotational semantics
\end{itemize}

\begin{itemize}
    \item The basis of our investigation is \cite{Kavvos2017b}'s $\PP$-categories, where morphisms are endowed with two kinds of equality, which we can use to model intensional and extensional equalities.
    \item We revisit elements of the theory of realisability in this setting, and construct a $\PP$-category of assemblies on $\lambda$-calculus and trackable maps, where trackable maps consist of not only a function but also its intensional description as part of the structure.
    
    \item Roughly speaking, an assembly $X$ on $\lambda$-calculus is a set $\bproj{X}$ of values, i.e.\ extensions, equipped with some description(s), i.e.\ intensions, in the form of $\lambda$-terms; the existence of such descriptions is merely a property that the values should satisfy, but the descriptions are not part of the structure and do not need to be explicitly presented.
    
    \item  The denotation of a code in the sense of $\boxtimes X$ is not just a description but a pair of a value~$x$ and its description $M$, where the description is made part of the structure.
    Further, the description of such a pair $(M, x)$ is a $\lambda$-term $N$ reducible to the \emph{Gödel code} of $M$.
    We can then validate the \SFour axioms, for example $\boxtimes X \to X$ (natural in~$X$) by just projection.
    The choice of using the Gödel code of~$M$ (rather than just~$M$ as chosen by Kavvos; see related work) as the description of $(M, x)$ prevents $X \to \boxtimes X$ from being natural in $X$ (so there is no generic internal quoting operation)---otherwise $\boxtimes X$ would be equivalent to~$X$ and become redundant from the view of modal logic.
    \item The denotation $(M, x)$ of a code in the sense of $\Box X$ is similar to $\boxtimes X$ but the value~$x$ is only available in a `later' stage.
    Guarded type theory has a built-in \emph{later} modality~$\laterp$ and guarded recursion $(\laterp X \to X) \to X$ available to use.
    Hence, for convenience, to give the denotational semantics of $\Box X$, we work within guarded type theory in order to use guarded recursion to model intensional recursion.
    It is still not possible for $X \to \Box X$ to be natural in~$X$ for the same reason as the case of $\boxtimes$, whereas $\Box X \to X$ can no longer be validated by projection (because the value from $(M, x)$ is not available now but later), and in fact will be shown impossible.
    
    \item The two aforementioned constructions are not $\PP$-functors (but exposures~\cite{Kavvos2017b}) so that congruence (preservation of extensional equality) is not imposed by definition and is even shown to be false.
    
    \item In short our denotational semantics is tight enough to capture the essence of intensionality by refuting congruence and internal quoting, which previous work did not achieve.
    \ltnote{Ask Kavvos}
\end{itemize}

\begin{itemize}
    \item As a minor issue, we develop our theory in homotopy type theory (HoTT) mainly for convenience.
    
    \item HoTT provides a high level of granularity of expressiveness, e.g., the clear difference between property versus structure that we need for defining various notions above. 
    
    \item We can work with a more structured version of sets within HoTT; in particular, equality between set elements can be handled much more naturally and conveniently, unlike in other type-theoretic foundations.
    
    \item Due to advances in formalisation of mathematics, our arguments are fully formalised and checked by the proof assistant Agda with the aid of the community effort (the cubical library, PLFA, etc.).
    \item Guarded type theory we use is also partially implemented in Agda.
\end{itemize}
...
This work is based on untyped $\lambda$-calculus, homotopy type theory, and $\PP$-categories.
Most, if not all, of the materials are standard, so we do not go into details but only recall what we need in the rest of this section.

\subsection{Homotopy type theory}

The mathematical foundation of our work is homotopy type theory (HoTT)~\cite{hottbook}.
...
This infinite-dimensional structure of equality types can be seen as an abstract form of homotopy theory, where types are interpreted as spaces and equality proofs as paths; in particular, equality types are path spaces, and paths between paths are homotopies.
Our work is built upon the reformulated set theory and logic within HoTT~\cite[Chapter~3]{hottbook}, where the equality structure degenerates at higher dimensions, but is still rich enough for us to be precise about notions such as sets and propositions, and existence with explicit witnesses versus `mere' existence.
Moreover, since implementations of HoTT are readily available, we are able to formalise and verify our constructions, for which HoTT provides essential features that some previous type theories lack, notably function extensionality: $\left(\prd{x : A} f\,x =_B g\,x\right) \to f =_{A \to B} g$.
...
In this paper we will work exclusively with \emph{sets}, which are types whose equality types are all propositions, so for any two inhabitants of a set, there is at most one way for them to be equal.
It is easy to check that the types we work with are sets: types constructed from sets and the type formers $\times$, $+$, $\to$, $\prod$, and~$\sum$ are all sets, and \emph{Hedberg's theorem}~\cite[Section~7.2]{hottbook} says that if a type~$A$ has decidable equality---that is, $\prd{x, y : A} (x =_A y) + \neg(x =_A y)$---then $A$~is a set.
...
For $\lambda$-calculus, we fix notations and state a few properties in homotopy type theory.
...
Contrary to the well-known first recursion theorem, Kleene's second recursion theorem works for \emph{code} instead of values and will be used to model the \GL modality partly.
...
\begin{definition}\label{def:trackable}
  Given assemblies $X$ and $Y$, a \emph{trackable map}~$f$ from $X$ to $Y$ consists of a function $\bproj{f}$ from $|X|$ to $|Y|$ and a term $F : \Lambda_1$ such that $F[M] \Vdash \bproj{f}\,x$ if $M \Vdash x$.
...
As we are only concerned about $\PP$-categories, we do not elaborate on the details here.
...
For notational convenience, $\laterp (\alpha : \kappa).\,A$ is written as $\laterp^\kappa\, A$ if the tick variable is not referred to.
...
This \GL modality then partially answers the question following Theorem~5.38 in the work of Kavvos~\cite{Kavvos2020} as one of the main contributions of this paper.
