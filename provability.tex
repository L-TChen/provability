...
This tick~$\diamond$ is needed to type Atkey and McBride's~\cite{Atkey2013b} operator
  ...
  Finally, every function $t : \laterp^\kappa A \to A$ has a delayed fixed point $\mathsf{dfix}^\kappa\,t : \laterp^\kappa A$ with a path replacing the judgemental fixed point equation (to ensure the termination of reduction semantics); this is called \emph{guarded recursion}, also known as \emph{Löb induction}:
\[
  \begin{minipage}{0.47\linewidth}
    \begin{prooftree}
