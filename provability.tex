...
The normality condition is realised exactly by~$\mathtt{ap}$:
\begin{proposition}
...
Given an element~$a$ of~$X$, we define its \emph{quotation} as the element $\boxtimes a \circ \bigstar$ of~$\boxtimes X$.
We say that a trackable map $q\colon X \to \boxtimes X$ \emph{quotes} an element $a$ of~$X$ if
\[
  \xymatrix{
...
%  \]
\end{proposition}
The fact that $\delta_X$ quotes closed elements, thoughts as a reduction $\mathtt{quote}[a/x] \mapsto \qquote{a}$, justifies the computational meaning categorically.
...
On the other hand, it is impossible to give a computational meaning of the rule $A \to \boxtimes A$ to hold for arbitrary~$A$.
...
\begin{theorem}\label{thm:S4-no-quoting}
  There are no trackable maps $\eta_\Lambda\colon \Lambda \to \boxtimes \Lambda$ and $\eta_\top\colon \top \to \boxtimes \top$ satisfying $\eta_\Lambda \circ a \sim \boxtimes a \circ \eta_\top$ for closed elements~$a$ of $\Lambda$.
  In particular, there is no natural transformation $\eta$ from $\mathcal{I}$ to $\boxtimes$.
\end{theorem}
\begin{proof}
  Assume $\eta_\Lambda$ and $\eta_\top$ satisfying that $\eta_X \circ a \sim \boxtimes a \circ \eta_\top$ for any $a \colon \top \to \Lambda$ given by \Cref{lem:global-element}.
...
Similar to~\Cref{thm:S4-no-quoting}, there is no family of trackable maps $\eta_X\colon X \to \Box^\kappa X$ natural in $X$ (or even just any morphisms $\eta_\top$ and $\eta_\Lambda$ satisfying $\eta_\Lambda \circ a \sim \boxtimes a \circ \eta_\top$ for elements of $\Lambda$) by the non-existence of a term $\mathtt{Q}$ with $\mathtt{Q}\,M \reduce \qquote{M}$ for any arbitrary term $M$.
...
\begin{proof}
  ...
  Now we prove the theorem statement by Löb induction on a $\Sigma$-type (\Cref{coro:lob-induction-sum}):
  assuming 
  \[
    x : \laterp \bproj{X}
    \quad\text{and}\quad
