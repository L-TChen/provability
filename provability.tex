...
\end{theorem}
Recall the Second Recursion Theorem (\Cref{coro:variant-of-SRT}) every term $F : \Lambda_1$ has a fixpoint  $\mathsf{fix}\,F$ such that $\mathsf{fix}\,F \reduce F [ \qquote{\mathsf{fix}\,F} ]$.
The theorem follows from the Löb induction and the fact that $\mathsf{fix}\,F$ is a fixpoint of $F$.
