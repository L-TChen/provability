...
\ltnote{(mere) proposition, set, structure vs.\ property, $n$-types are closed under $\sum$, $\prod$-type formers, \setU, \propU}
\ltnote{function extensionality}
...
The set~$M \reduce N$ of reductions from $M$ to $N$ consists of sequences of witnesses such as $\beta : (\lambda \mathtt{x}.\,M)\,N \onereduce M [ N / \mathtt{x} ]$.
...
Instead of ordinary categories, we work with $\PP$-categories which are pioneered by \v{C}ubri\'c et al.~\cite{Cubric1998a} to study normalisation in typed $\lambda$-calculus.
...
However, with functional extensionality, any inhabitant of the alternative formulation leads to a \emph{contradiction}~\cite{Troelstra1977}. \ltnote{prove it formally}
...
Hitherto, we complete our cook's tour of the previous results (based on a partial combinatory algebra, though) in HoTT.
