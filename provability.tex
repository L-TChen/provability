...
A clock quantification~$\forall \kappa.\, A$ introduces a clock to a context with rules similar to those for $\Pi$-types:
\[
  \begin{minipage}{0.47\linewidth}
    \begin{prooftree}
...
An inhabitant of $\laterp (\alpha : \kappa).\, A$ can tick on a different clock by a constant~$\diamond$ stated by 
\begin{prooftree}
...
This tick~$\diamond$ is needed to type Atkey and McBride's~\cite{Atkey2013b} operator
  ...
  Finally, every function $t : \laterp^\kappa A \to A$ has a \emph{delayed fixed point} $\mathsf{dfix}\,t : \laterp^\kappa A$ with a path replacing the judgemental fixed point equation (to ensure the termination of reduction semantics), called guarded recursion.
...
\begin{proposition}\label{lem:later-identity}
  Let $x$ and $y$ be inhabitants of $A$. Then, $\forall \kappa.\, \left(\mathtt{next}\,\kappa\,x_1 = \mathtt{next}\,\kappa\,x_2\right)$ implies $\forall \kappa.\, x_1 = x_2$.
\end{proposition}
Here is a caveat, though.
The above proposition cannot hold within the same clock, i.e. $\forall \kappa.\, \left(\mathtt{next}\,\kappa\,x_1 = \mathtt{next}\,\kappa\,x_2\right) \to \left(x_1 =x_2\right)$ is not provable, since intuitively it would require us to retrieve inhabitants available later \emph{right now}.
In order to formulate faithfulness of an exposure, the notion of exposures is adopted with a clock variable accordingly to express the faithfulness with respect to~$\sim$ properly:
