...
However, by inspecting Troelstra's argument~\cite{Troelstra1977} (also Forster's Coq formalisation~\cite{forster2021}) one can show that \eqref{eq:alternative-right-totality} for $\bproj{X} = \Nat \to \Nat$ allows us to define 
\begin{equation}\label{eq:all-zeros-is-decidable}
  \prd{f : \Nat \to \Nat} \mathsf{Dec}\left(f = (\lambda x.\, 0)\right)
\end{equation}
using function extensionality and confluence of $\lambda$-calculus where natural numbers are realised by terms reducible to Church numerals.
Clearly, \eqref{eq:all-zeros-is-decidable} is empty, so \eqref{eq:alternative-right-totality} is not adopted.
