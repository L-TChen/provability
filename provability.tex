...
Here we only intuitively introduce the constructs and properties of CCTT that are necessary for the informal presentation of the constructions in \Cref{sec:GL}, but the formal details are all checked in \textsc{Agda} in the guarded cubical mode introduced by Veltri and Vezzosi~\cite{Veltri2020}.
...
\end{proof}
By inspecting the above proof, we can see that the tracker of~$\Box^\kappa X \to X$ is essential, since it is used to construct the fixpoint~$\mathsf{fix}\,F$. 
...
\end{theorem}
The strong Löb axiom as a map from $\Box^\kappa X \Rightarrow X$ to $X$, without the access to the intension, is not sufficient to be tracked by the SRT, justifying the interpretation of \GL as \emph{intensional} recursion.
