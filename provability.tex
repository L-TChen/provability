...
Set-theoretically, a trackable map between assemblies on a PCA $(A, \cdot)$ is a function~$f$ \emph{merely} tracked by some $b \in A$ in the sense that there merely exists some $b$ such that $b \cdot a \Vdash f\,x$ whenever $a \Vdash x$.
In this case, $b$ is called the \emph{tracker} of $f$.
Equivalently, we may also say that $f$ is tracked by $b$.
...
In the setting based on a PCA, every element $x$ of (the carrier of) an assembly $X$ corresponds to a trackable map from $\top$ to $X$, i.e.\ a \emph{global element} of $X$, defined as
\[
  f(\star) = x
  \quad\text{merely tracked by}\quad
  \mathsf{k}\cdot a 
\]
where $a$ is a realiser of $x$ (whose mere existence is given the right totality) and $\mathsf{k}$ is the first projection.
Therefore, the terminal object is a \emph{separator} (or called generator~\cite{MacLane1994}) in the sense that two trackable maps $f, g \colon X \to Y$ are distinct only if there exists some $x \colon \top \to X$ such that $f\circ x \neq g \circ x$.

However, as the tracker~$F$ is part of a trackable map $(\bproj{f}, F, \mathfrak{f})$ in this type-theoretic setting, we cannot construct a global element for any inhabitant $x : \bproj{X}$ without further information.
Accordingly, we strengthen the assumption to define a global element. 
\begin{lemma}\label{lem:global-elemnt}
  For every assembly $X$, every inhabitant $x_0 : \bproj{X}$ with a realiser $M_0$ and its witness $r_0 : M \Vdash_X x$ of realisability gives rise to a global element of $X$:
  \[
    (\mathsf{c}_{x_0}, M_0, \lambda M\,x\,r.\, r_0)
  \]
  where $\mathsf{c}$ is the constant function with the value $x_0$.
\end{lemma}

On the other hand, if the carrier of a codomain $Y$ does not have non-trivial higher paths, then global elements are sufficient to separate distinct maps.
\begin{proposition}
...
\begin{definition}[Clocked-indexed exposure]
...
The only difference between carriers $\bproj{\Box X}$ and $\bproj{\boxtimes X}$ is the extensional part which becomes $\rhd\bproj{X}$ so that the extension of $(M, x, \star)$ is available in the next stage but not earlier.
...
The payoff for disallowing (run-time) evaluation is to be able to apply guarded recursion for typed code
which is tracked by the fixpoint operator from Kleene's second recursion theorem. From the logical perspective, it is exactly the Gödel-Löb axiom.

\begin{theorem}[{Guarded recursion for $\Box A$}]\label{thm:GL-recursion}
  For arbitrary map $f \colon \Box X \to X$ tracked by some $F$, there exists a global element of $\Box X$ tracked by $F[ \qquote{\mathsf{fix}(\lambda \mathtt{x}.\,F)}/\mathtt{x} ]$.
\end{theorem}
The construction of the global element essentially has two parts: one apply the guarded recursion for the later modality and the Kleene's second recursion theorem on the extensional and the intensional parts of $(\bproj{f}, F, \mathfrak{f})$ respectively. Then, we use the fact $\bproj{f}$ is tracked by $F$ and $\mathsf{fix}\,F$ reduces to $F [ \qquote{\mathsf{fix}(\lambda \mathtt{x}.\, F}) / \mathtt{x} ]$ to conclude the theorem.
\begin{proof}
  Let $f$ be a trackable map from $\Box X$ to $X$.
  Assume an inhabitant $x^{\rhd}$ of type $\rhd \sum_{x : \bproj{X}} F [ \qquote{ \mathsf{fix}\,(\lambda \mathtt{x}.\, F) } / \mathtt{x} ] \Vdash x$.
  
  Therefore, by \Cref{lem:global-elemnt}, the inhabitant of $\sum_{x : \bproj{X}} F[ \qquote{\mathsf{fix}\,(\lambda \mathtt{x}.\, F)} / \mathtt{x} ] \Vdash_X x$ gives rise to a global element of $X$ as desired.
\end{proof}

\begin{corollary}[Gödel-Löb axiom]
...
\begin{enumerate}
  ...
  \item Consistent version of \cite{Kavvos2017a}
