...
Indeed, with functional extensionality which holds in homotopy type theory, any inhabitant of the alternative formulation leads to a contradiction~\cite{Troelstra1977}. \ltnote{check this}
...
\begin{example}
  Another useful assembly which plays a significant role in~\Cref{sec:provability} is the type $\Lambda_0$ of closed terms itself with $\Vdash_\Lambda$ defined by
  \[
...
\ltnote{use the fact that there is a lifted path from $x$ to the transported one}
...
\end{corollary}
\begin{proof}
  This statement follows from~\Cref{prop:category-of-assemblies}.
\end{proof}
