...
\acknowledgements{%
  ...
  We would also like to thank Rasmus Ejlers Møgelberg, Churn-Jung Liau, Ren-June Wang, Tom de Jong, Chad Nester, Anton Setzer, and Andrea Vezzosi for useful exchanges.
  Finally, we thanks anonymous reviewers for their suggestions and thoughtful comments which are encouraging to us.}
...
\ltnote{definitions of modal logics}
...
Indeed, our work has been formalised in \textsc{Agda}~2.6.2~\cite{Agda} which also supports guarded type theory in the guarded cubical mode~\cite{Veltri2020}.%
\hsnote{cubical implementation of HoTT, clocked cubical type theory (for forcing), no native cubical arguments}
...
\hsnote{rewrite as notation fixing?}
...
\hsnote{Hedberg's theorem for our own sets}
...
\hsnote{restructure into two sections}
...
\ltnote{categorical simulations: see \cref{sec:related-work}}
\end{remark}
...
\end{proof}
By inspecting the above proof, we can see that the intension~$F$ of~$\Box^\kappa X \to X$ is essential, since $F$ is used to construct the fixpoint~$\mathsf{fix}\,F$. 
To internalise the inductive form as the \GL axiom, we will also need the intension:%
\ltnote{no strong Löb}
...
\end{theorem}
The strong Löb axiom as a map from $\Box^\kappa X \Rightarrow X$ to $X$, without the access to the intension, cannot be tracked by the SRT, justifying the interpretation of \GL as \emph{intensional} recursion.
...
The difference between $\boxtimes_K$ and $\boxtimes$ mainly comes from the choice of realisability and the use of Gödel encoding.%
\ltnote{no technical difference due to the choice of metalanguage... Well, not really.}
...
Finally, a generic quoting for~$\boxtimes_K$ \emph{can} be defined on the trivial one-element PCA. 
Assuming the axiom of choice, there (merely) exists a function~$q$ from $ \bproj{X}$ to $\bproj{\boxtimes_K X}$.
Hence, we conjecture that~$q$ could be realised in the sense of Krivine's classical realisability~\cite{Krivine2021}, which would establish a non-trivial example of generic quoting for~$\boxtimes_K$.
...
\ltnote{Semantical Analysis of Contextual Types by Brigitte Pientka and Ulrich Schöpp. I don't understand logical framework and their semantics.}
