...
A $\PP$-category has two kinds of equality for morphisms---the underlying identity $=$ and the partial equivalence relation $\sim$, where the former is considered as the \emph{intensional equality} and the latter as the \emph{extensional equality}.
...
%\begin{definition}
%  Given exposures $P, Q\colon \mathbf{C} \looparrowright \mathbf{D}$, a \emph{(natural) transformation of exposures} $t\colon P \to Q$ is a family of well-defined morphisms $t_X \colon PX \to QX$ such that for every well-defined morphism~$f\colon X \to Y$ the following diagram
%  \[
%    \xymatrix{
...
%\end{definition}
We may further define the notion of natural transformations for exposures and comonadic exposures as introduced by Kavvos~\cite{Kavvos2017b} to formulate the \SFour modality.
However, in our framework, we would have to adopt the \emph{oplax} version instead, where the naturality only holds up to $\reduce$.
...
\hsnote{forward reference to Remark 4.3}
...
\begin{proof}[Proof sketch]
  It is not hard to prove that $\boxtimes$ preserves identities and composition.
...
\]
  ...
  By assumption the path type $\bproj{f}\,x =_Y \bproj{g}\,x$ is a proposition and there merely exists $(M ,r) : \sm{M : \Lambda_0} M \Vdash_X x$, we can apply
  the recursion principle of propositional truncation to prove
  \[
...
\begin{remark}
  ...
  \ltnote{discuss K axioms here}

  In terms of our $\boxtimes$, there are countably many representations for $\star : \Unit$ in $\boxtimes \top$, so clearly $\boxtimes \top$ is not $\PP$-isomorphic to $\top$. 
Also, $\boxtimes (X \times Y)$ is not $\PP$-isomorphic to $\boxtimes X \times \boxtimes Y$ by the same reason.
...
One may expect that $(\boxtimes, \epsilon, \delta)$ is comonadic in the sense that $\epsilon$ and $\delta$ are natural up to~$\sim$ satisfying comonad laws.
Unfortunately $\delta_X$ is not even natural in~$X$.
...
\begin{remark} \label{remark:difference-boxtimes}
  ...
  Preserving finite products are considered \emph{unsatisfactory} because intensions, i.e.\ elements of a PCA, are subject to certain equations already as some of different representations are already identified on the level of PCA.
  By taking $\lambda$-terms only subject to $\alpha$-equivalence as realisers, we are able to distinguish more extensionally equivalent but intensionally different inhabitants.
...
\begin{theorem}\label{thm:S4-no-quoting}
  \ltnote{can we weaken the premise? Use oplax naturality instead of ordinary naturality?}
  There is no family of trackable maps from $X$ to $\boxtimes X$ indexed by assemblies~$X$ satisfying the commutative diagram
  \begin{equation} \label{eq:naturality-quoting}
    \xymatrix{
      X \ar[r]^{\eta_X} \ar[d]_{f} & \boxtimes \ar[d]^{\boxtimes f} Y \\
      Y \ar[r]_{\eta_Y} & \boxtimes X
    }
  \end{equation}
  up to~$\sim$ for each trackable map $f\colon X \to Y$.
\end{theorem}
...
\begin{proof}
  ...
  Consider the naturality~\eqref{eq:naturality-quoting}, that is, ${\eta_\Lambda \circ \widehat{M_0}} \sim {\boxtimes \widehat{M_0} \circ \eta_\top}$ when $f = \widehat{M_0}$.
  Note that $\boxtimes \widehat{M_0}$ maps an arbitrary inhabitant of $\boxtimes \top$ to $(M_0, M_0, \refl_{\twoheadrightarrow})$ so that $\bproj{\eta}$ must satisfy that for each trackable map $f$ 
  \begin{equation} \label{eq:naturality-at-lambda}
    \bproj{\eta_\Lambda}\,M_0 \equiv \left(\bproj{\eta_\Lambda} \circ \widehat{M_0}\right)\,\star = \left(\boxtimes \widehat{M_0} \circ \eta_\top \right)\,\star \equiv (M_0, M_0, \refl_{\twoheadrightarrow}).
  \end{equation}
...
\end{proof}
\begin{remark}
  The above theorem can be further strengthened by weakening the naturality.
  Indeed, if we only require $\eta$ to satisfy
  \[
    \boxtimes f \circ \eta_X (x) \leq \eta_Y \circ f (x)
  \]
  where $(M, x, r) \leq (N, y, s)$ holds if $x = y$ and $M \reduce N$. 
  Then reconsider~\eqref{eq:naturality-at-lambda} with $M_0 = \mathtt{I}$ and $\mathtt{I I}$.
  By assumption $\bproj{\eta_\Lambda}\,\mathtt{I} = (N , \mathtt{I}, r) \geq (\mathtt{I}, \mathtt{I}, \refl_{\twoheadrightarrow})$ so that $\mathtt{I} \reduce N$ and thus $N =_\alpha \mathtt{I}$. 
  Similarly, one can show that $\bproj{\eta_\Lambda}\,(\mathtt{I}\,\mathtt{I}) = (\mathtt{I}\,\mathtt{I}, \mathtt{I}\,\mathtt{I}, \refl_{\twoheadrightarrow})$.
  Then, we can proceed with the same argument in \Cref{subsec:godel} to show that the tracker of $\eta_\Lambda$ cannot exist.
  
\end{remark}

It is unclear if there is any way to prove the above theorem without resorting to the non-existence of $\Quote'$ in $\lambda$-calculus and naturality.
Likewise, it remains unclear to us if the existence of a natural transformation $\mathcal{I} \to \boxtimes_K$ can be refuted.
...
\begin{proof}
  ...
  \end{enumerate}
  Therefore $(F[\qquote{\mathsf{fix}\,F}], x_0, r_0)$ is an inhabitant of $\Box^\kappa X$ which evidently is realised by $\qquote{F[\qquote{\mathsf{fix}\,F}]}$ and thus by \Cref{lem:global-elemnt} it gives rise to a global element~$f^\dagger$ of $\Box X$, concluding our proof.
\end{proof}

We may wish to prove further that the global element $f^\dagger$ is indeed a fixpoint of $f$ in the sense that $f^\dagger \sim \Box(f \circ f^\dagger) \circ \bigstar$ in line with Kavvos' definition~\cite[Definition~5.29]{Kavvos2020} where $\bigstar : \top \to \Box \top$ is a global element of $\Box \top$ given by $\star : \Unit$ and its realiser $\lambda \mathtt{x}.\,\mathtt{x}$ using~\Cref{lem:global-elemnt}.
