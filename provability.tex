...
\ltnote{(mere) proposition, set, structure vs.\ property, $n$-types are closed under $\sum$, $\prod$-type formers}
...
The type of terms is denoted by $\Lambda$ and the type of terms of at most $n$ many free variables by $\Lambda_n$.
In particular, $\Lambda_0$ is the type of \emph{closed terms}.
We write $F[M]$ instead of $F[M/\mathtt{x}]$ for $F : \Lambda_1$ if there is no danger of confusion since $F$ has at most one free variable.
...
%\end{definition}
We may further define the notion of natural transformations for exposure and comonadic exposures as introduced by Kavvos~\cite{Kavvos2017b}.
However, in our setting, we would have to adopt the \emph{oplax} version instead where the naturality does not holds strictly but with respect to $\reduce$.
...
\begin{definition}\label{def:assembly}
  ...
  \[
    \ASM_0(\Lambda) \defeq \sm{\bproj{X} : {\Univ}}{\mathord{\Vdash_X}:\Lambda_0 \to |X| \to \Univ} 
  ...
  \end{align}
  A \emph{set-theoretic assembly} is an assembly $X$ whose carrier $\bproj{X}$ is a set and $M \Vdash_X x$ is a relation for any $M$ and $x$.
  The type of set-theoretic assemblies is denoted by $\Asm_0(\Lambda)$. 
\end{definition}
...
However, with functional extensionality which holds in HoTT, any inhabitant of the alternative formulation leads to a \emph{contradiction}~\cite{Troelstra1977}. \ltnote{prove it formally}
...
\begin{definition}\label{def:trackable}
  ...
  In other words, the type of trackable maps is $\ASM_1(X, Y) \defeq \sum_{\bproj{f} : |X| \to |Y|} \sum_{F : \Lambda_1} \mathsf{Tracks}_{X, Y}(F, \bproj{f})$ where
  \[
    \mathsf{Tracks}_{X, Y}(F, \bproj{f}) \defeq \prd{M : \Lambda_0}{x : |X|}
    \left(M \Vdash_X x\right) \to \left(F[M/\mathtt{x}] \Vdash_Y \bproj{f}\, x\right).
  \]
  A \emph{merely trackable map} is an inhabitant of $\sm{\bproj{f} : |X| \to |Y|} \exis{F : \Lambda_1}\mathsf{Tracks}_{X, Y}(F, \bproj{f})$. 
\end{definition}
Denote by $\Asm_1(X, Y)$ if $X$ and $Y$ are set-theoretic.
...
As we are only concerned about $\PP$-categories, we do not elaborate details here.
...
\end{proposition}
In addition, we denote by $\Asm(\Lambda)$ the $\PP$-category of assemblies whose carrier is a set and the realisability relation $\Vdash$ is a mere relation.
...
Moreover, since sets are closed under $\sum$-types and $\prod$-types, these constructions carry over to~$\Asm(\Lambda)$.
\begin{corollary}
...
In this section, we first introduce a comonadic exposure $\boxtimes\colon \ASM(\Lambda) \looparrowright \ASM(\Lambda)$ which models a $S4$ modality but subtly different from the exposure $\boxtimes_K$ introduced by Kavvos~\cite{Kavvos2017b}.
