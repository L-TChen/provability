...
Note that the type $\Lambda$ of terms has decidable equality as well as $M \reduce N$, so they are sets. 
...
Its use is lately advocated by Kavvos~\cite{Kavvos2017b} who introduced a functor-like concept called \emph{exposure} exemplified by the Second Recursion Theorem and more in the realm of computability theory.
...
The composite of two $\PP$-functions by function composition is a $\PP$-function.
...
This additional equality~$\sim$ reflects the fact that, for example, $\alpha$-equivalent terms are $\beta$-equivalent but not the converse.
For categorical semantics where terms are interpreted as morphisms, an interpretation into a $\PP$-category is possible to discriminate these two kinds of identities, enabling us to model intensionality.
...
In this section, we first introduce an exposure $\boxtimes\colon \Asm(\Lambda) \looparrowright \Asm(\Lambda)$ which models a \SFour modality but subtly different from the comonadic exposure $\boxtimes_K$ introduced by Kavvos~\cite{Kavvos2017b}.
...
\]
  ...
  \]
  Therefore, we have $\prd{x : \bproj{X}} \to \left(\bproj{f}\,x =_Y \bproj{g}\,x\right)$.
...
Similar to~\Cref{thm:S4-no-quoting}, there is no family of trackable maps $\eta_X\colon X \to \Box X$ natural in $X$ by the non-existence of the term $\mathtt{Q}$ with $\mathtt{Q}\,M \reduce \qquote{M}$ for any arbitrary term $M$.
...
It is a natural setting when the underlying foundation is equipped with Löb induction (guarded recursion) as an abstract replacement of the step-indexing technique.
