...
Then, by function extensionality, any inhabitant~$G$ of~\eqref{eq:alternative-right-totality} for $\bproj{X} = \Nat \to \Nat$ would allow us to decide whether a function~$f$ is propositionally equal to a constant function or not by comparing their realisers $Gf$ and~$G(\lambda x.\, 0)$ as shown by Troelstra~\cite{Troelstra1977,forster2021}, which is not possible.
