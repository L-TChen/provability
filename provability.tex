...
Metaprogramming is about writing (meta)programs that manipulate programs.
...
Therefore, there have been investigations into type-theoretic foundations for staged computation using temporal modality, \SFour modality~\cite{Davies2001b}, contextual \SFour modality, \GL modality~\cite{Kavvos2017a}, and so on, which have influenced the design of practical implementations to varying degrees.\ltnote{cite staged programming languages}
...
Recently, Kavvos proposed a `new' construct called \emph{intensional recursion}~\cite{Kavvos2017a} for $\mathtt{Code}$ that can be used to construct a value recursively from its own code (intension), extending the capabilities of metaprogramming.
In contrast to general recursion $(A \to A) \to A$, which also constructs a value recursively but does not inspect its own code but only its value (extension), intensional recursion alone does not lead to logical inconsistency.
...
Intuitively, while both modalities model code and may appear similar, there are still crucial differences:
in general, programs are to be evaluated in a \emph{next} stage, but \SFour is a special case where the next stages include the current one, so in particular the result of evaluating a program can be immediately used in the current stage, as witnessed by $(\mathbf{T})$;
...
To illuminate the above intuition, we present in this paper a denotational semantics of two types of code, namely a type $\boxtimes A$ of code that can be evaluated in the current stage and a type $\Box A$ of code to be evaluated in a next stage.
Building on the previous work using $\PP$-categories~\cite{Cubric1998a,Kavvos2017b}, which allow us to distinguish between intensions and extensions, we revisit elements of \emph{realisability} in this setting, and construct a $\PP$-category of assemblies on $\lambda$-calculus and trackable maps, where trackable maps consist of not only a function but also its intensional description as part of the structure.
Roughly speaking, an assembly~$X$ on $\lambda$-calculus is a set $\bproj{X}$ of extensions associated with at least an intension in the form of a $\lambda$-term; the existence of such an intension is merely a property that the extension should satisfy, but the intension is not part of the structure.
In contrast, the denotations $\boxtimes X$ and $\Box X$ of the two types of code both consist of pairs~$(M, x)$ of an extension~$x$ and an associated intension~$M$, which is made part of the structure; the associated intensions of $(M, x)$ required to (merely) exist for assemblies are $\lambda$-terms~$N$ that are reducible to the Gödel code~$\qquote{M}$ of~$M$.
The choice of using the Gödel code of~$M$ (rather than just~$M$ as chosen by Kavvos~\cite{Kavvos2017b}---see related work) as the intensions of $(M, x)$ prevent $X \to \boxtimes X$ from being natural in~$X$ (so there is no generic internal quoting operation).
The subtle difference between $\boxtimes X$ and $\Box X$ is, however, where extensions are from.
\begin{itemize}
    ...
    By working in guarded typed theory, $\Box X \to X$ can no longer be validated by projection (because the value from $(M, x)$ is available later rather than now) and is, in fact, not possible; also intensional recursion can be modelled by guarded recursion.
\end{itemize}
The above constructions are not $\PP$-functors (but exposures~\cite{Kavvos2017b}) so that congruence (preservation of extensional equality) is not imposed by definition and is even shown to be false.
Hence, our denotational semantics is precise enough to capture the essence of intensionality by refuting internal quoting and congruence, which previous work fails to achieve.%
