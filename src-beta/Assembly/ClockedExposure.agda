...
record IsCloExpo (Q : Cl → Asm 𝓤 → Asm 𝓤) (map : {X Y : Asm 𝓤} → (k : Cl) → Trackable X Y → Trackable (Q k X) (Q k Y)) : 𝓤 ⁺ ̇ where 
  field
    preserve-id   : {k : Cl} → (X : Asm 𝓤)
      → map k (id X) ∼ id (Q k X) ꞉ Q k X →ₐ Q k X
    preserve-comp : {k : Cl} (f : Trackable X Y) (g : Trackable Y Z)
      → map k (g ∘ f) ∼ map k g ∘ map k f ꞉ Q k X →ₐ Q k Z
    reflects-∼    : {k : Cl} (f g : Trackable X Y)
      → map k f ∼ map k g ꞉ Q k X →ₐ Q k Y
      →     f ∼ g     ꞉ X   →ₐ Y
...
Naturality {𝓤} {k} P Q fun = {X Y : Asm 𝓤} → (f : Trackable X Y) → fun ∘ P .map k f ∼ Q .map k f ∘ fun ꞉ P .obj k X →ₐ Q .obj k Y
