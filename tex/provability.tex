...
\acknowledgements{%
  ...
  We would also like to thank Jacques Carette, Martín Escardó, Tom de Jong, Churn-Jung Liau, Rasmus Ejlers Møgelberg, Chad Nester, Anton Setzer, Andrea Vezzosi, Ren-June Wang, and Zhixuan Yang for useful exchanges.
...
For instance, in $\lambda$-calculus, it is well-known~\cite{Barendregt1984a} that there are terms encoding the Gödel code of a code and evaluating a code respectively.

Recently, Kavvos proposed \emph{intensional recursion}~\cite{Kavvos2021} for $\mathtt{Code}$ to construct a value recursively from its own code (intension).
Logically, intensional recursion amounts to the \textbf{G}ödel-\textbf{L}öb axiom $\Box (\Box A \to A) \to \Box A$ for the modal logic \GL~\cite{Boolos-provability}, where $\Box A$ stands for `$A$~is provable'.
...
Indeed, our work has been formalised in \textsc{Agda}~\cite{Agda}, which implements HoTT and guarded type theory respectively in the forms of cubical type theory~\cite{Cohen2015, Vezzosi-Cubical-Agda} and ticked cubical type theory~\cite{Veltri2020} with clock quantification~\cite{Kristensen2021}. However, we do not use cubical arguments.
...
HoTT identifies this infinite-dimensional structure of equality types as an abstract form of homotopy theory, where types are interpreted as spaces and equality proofs as paths; in particular, equality types are path spaces, and paths between paths are homotopies.
We do not make use of the full generality of HoTT but work exclusively with \emph{propositions} and \emph{sets}, whose equality structure degenerates at higher dimensions.
...
If $P$~and~$Q$ are propositions, then so is their product $P \times Q$.
Similarly, if a family $R(x)$ of types indexed by $x : A$ (where $A$~is any type) are all propositions, then so is the product $\prd{x : A} R(x)$.
Logically these give us conjunction and universal quantification; therefore we may write $\fall{x : A} R(x)$ in place of $\prd{x : A} R(x)$.
...
With truncation, we introduce a propositional version of existential quantification by defining $\exis{x : A} R(x) \defeq \brck{\sm{x : A} R(x)}$, from which we cannot project a witness~$x$ and a proof of $R(x)$ in general, but can only derive another proposition~$P$ provided that $\left(\sm{x : A} R(x)\right) \to P$.
